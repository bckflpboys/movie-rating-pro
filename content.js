...
if (window.hasRun) {
    // If the script has already run, we don't need to add listeners again
    // But we might want to re-run detection if needed, though the listener handles that.
    // For now, just return to avoid duplicate listeners.
    // However, we need to make sure the listener is still active.
    // Since window.hasRun is on the window object, the previous script execution persists.
} else {
    window.hasRun = true;
